...
```
`userId` is whatever `getCurrentUserId()` in the frontend resolves to. Inside
Telegram that's the Telegram user id prefixed with `tg-`, so it lines up with
Telegram Mini App auth automatically. Outside Telegram (someone using the
site as a plain website via `login.html` / `signup.html`), it's a generated
id prefixed with `web-`, and the document also carries:
```
{
  "name": "Rahim",
...
```
Password is bcrypt-hashed by `/api/auth/signup`, same as admin passwords -
never stored or transmitted in plaintext. Session cookie: `ac_user_session`
(separate from the admin's `ac_admin_session`, so one can never be used as
the other).
