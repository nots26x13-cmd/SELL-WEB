# S26X CODEX SHOP - storefront, wallet & admin panel
...
Inside Telegram, people never see a login screen - `getCurrentUserId()`
already uses their Telegram id automatically. `frontend/login.html` and
`frontend/signup.html` are for the case where someone opens the site as a
plain website outside Telegram: email + password, session kept in an
httpOnly cookie (`ac_user_session`), same bcrypt+JWT pattern as the admin
login but on a separate cookie/role so one can never be mistaken for the
other. Backend routes: `POST /api/auth/signup`, `POST /api/auth/login`,
`POST /api/auth/logout`, `GET /api/auth/me` (`backend/app/routers/auth.py`).
`account.html` shows a "log in / sign up" prompt for guests and a
name/email + log-out button once signed in.

### About the admin bootstrap password

`backend/.env` in this zip already has `ADMIN_BOOTSTRAP_EMAIL` /
`ADMIN_BOOTSTRAP_PASSWORD` filled in with the values you gave in chat. Same
rule as before: that password has now been typed in a chat more than once,
so treat it as semi-public - log in once, then immediately change it from
the admin panel (or pick a different one before your first deploy). Render
doesn't read `backend/.env` - set the same two keys under the dashboard's
